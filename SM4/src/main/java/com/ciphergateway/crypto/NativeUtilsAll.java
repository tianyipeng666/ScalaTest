...
public class NativeUtilsAll {

    private NativeUtilsAll() {}
    ...
    private static void loadFromResource(String resourcePath, String fileName) {
        try (InputStream in = NativeUtilsAll.class.getResourceAsStream(resourcePath)) {
            if (in == null) {
    ...
    private static void lazyCleanOnce() {
        ...
        synchronized (NativeUtilsAll.class) {
            if (cleanedOnce) return;
